...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func favoriteTweet(tweetId:Int, sucess:@escaping () -> (), failure: @escaping (Error) -> ())
    {
        ...
        TwitterAPICaller.client?.post(url, parameters: ["id":tweetId], progress:nil, success: {(task: URLSessionDataTask, response: Any?) in sucess()
    ...
    func rewweet(tweetId:Int, sucess:@escaping () -> (), failure: @escaping (Error) -> ())
       {
